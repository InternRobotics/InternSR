...
def build_model_from_config(cfg, model_name, use_vllm=False):
    ...
    if hasattr(vlm.api, cls_name):
        model = getattr(vlm.api, cls_name)(**config)
    elif hasattr(vlm, cls_name):
        model = getattr(vlm, cls_name)(**config)
    else:
        raise ValueError(f'Class {cls_name} is not supported in `vlm`')
...
def build_dataset_from_config(cfg, dataset_name):
    ...
    cls_name = config.pop('class')
    if hasattr(dataset, cls_name):
        cls = getattr(dataset, cls_name)
